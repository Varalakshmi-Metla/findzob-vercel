...
export default function HotJobsAdminPage() {
  ...
  const hotJobsCollectionRef = useMemo(() => firestore ? collection(firestore, 'hot-jobs-india') : null, [firestore]);
  const { data: hotJobs, isLoading: hotJobsLoading, error: hotJobsError } = useCollection<any>(hotJobsCollectionRef);

  const [newJob, setNewJob] = useState({ title: "", company: "", location: "", salary: "", type: "", logo: "", url: "", priority: false, tags: "" });
  ...
  const handleAddJob = async (): Promise<boolean> => {
    ...
    setNewJob({ title: "", company: "", location: "", salary: "", type: "", logo: "", url: "", priority: false, tags: "" });
    return true;
  };
            ...
            <div className="flex items-center gap-2">
                ...
                onChange={async (e) => {
                  ...
                  try {
                    ...
                    const toAdd = rows.map(r => ({
                      ...
                      url: cleanValue(r.url || r.URL || r.job_url || r.apply_url || r.link),
              ...
              <Button size="sm" onClick={() => {
                ...
                const rows = (hotJobs || []).map(h => ({
                  title: h.title || '', company: h.company || '', location: h.location || '', salary: h.salary || '', type: h.type || '', logo: h.logo || '', url: h.url || '', tags: Array.isArray(h.tags) ? h.tags.join(',') : (h.tags || ''), priority: h.priority ? 'true' : 'false'
                }));
                const headers = ['title','company','location','salary','type','logo','url','tags','priority'];
              ...
              <Button size="sm" onClick={() => {
                ...
                const headers = ['title','company','location','salary','type','logo','url','tags','priority'];
                const sample = [headers.join(','), 'Senior Software Engineer,Google,Mountain View,120000-150000,Full-time,,https://example.com/apply,AI/ML;Cloud,true'].join('\n');
          ...
          <Table>
            ...
            <TableBody>
              ...
              {!hotJobsLoading && filteredJobs.map(job => (
                editJobId === job.id ? (
                    ...
                    <TableCell colSpan={9}>
                      <div className="w-full">
                        <div className="flex-1">
                          ...
                          <input type="text" className="border rounded px-2 py-1 mb-1 w-full" value={editJob?.url || ''} onChange={e => setEditJob({ ...editJob, url: e.target.value })} placeholder="Job URL" />
                  ...
                  <TableRow key={job.id}>
                    ...
                    <TableCell>{job.url ? <a href={job.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline truncate">{job.url}</a> : '—'}</TableCell>
              ...
              {!hotJobsLoading && filteredJobs.length === 0 && (
                <TableRow>
                  <TableCell colSpan={9} className="h-48 text-center text-muted-foreground">No jobs found.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
